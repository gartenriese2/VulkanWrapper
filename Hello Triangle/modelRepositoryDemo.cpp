...
namespace bmvk
{
    ...
    void ModelRepositoryDemo::initModels()
    {
        ...
        const auto createSide = [&posToCol](std::vector<vw::scene::Vertex<VD_MRD>> & vertices, std::vector<uint32_t> & indices, const glm::vec3 & n, const glm::vec3 & a, const glm::vec3 & b, const glm::vec3 & c, const glm::vec3 & d, const int i)
        {
            vertices.push_back(vw::scene::Vertex<VD_MRD>{ a, n, posToCol(a) });
            vertices.push_back(vw::scene::Vertex<VD_MRD>{ b, n, posToCol(b) });
            vertices.push_back(vw::scene::Vertex<VD_MRD>{ c, n, posToCol(c) });
            vertices.push_back(vw::scene::Vertex<VD_MRD>{ d, n, posToCol(d) });
        ...
        std::vector<vw::scene::Vertex<VD_MRD>> vertices;
    ...
    void ModelRepositoryDemo::createPipelines()
    {
        ...
        auto bindingDescription = vw::scene::Vertex<VD_MRD>::getBindingDescription();
        auto attributeDescriptions = vw::scene::Vertex<VD_MRD>::getAttributeDescriptions();
